...
- [ ] 과제 마감 시각(due_at) 검증 테스트가 있다(remind_at 이후, now 이후)
- [ ] 임박알림 파싱(`24,3,1`) 및 중복 방지(pre_notified) 테스트가 있다
- [ ] 마감 시 CLOSED 상태 전이 및 closed_at 기록 테스트가 있다
- [ ] 알림역할 지정 시 역할 멘션 포함/미지정 시 멘션 생략 테스트가 있다
