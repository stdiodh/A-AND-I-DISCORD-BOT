...
- `FEATURE_TASK_QUICKREGISTER_V2` (기본 `true`)
...
- `HomeSlashCommandHandler`
  - `/홈 설치 [채널]`
  - HOME_V2에서 상태판 UI(오늘 상태 + 지금 해야 할 일) 제공
  - 상태별 1행 버튼:
...
- `MeetingSlashCommandHandler`
  ...
  - `/회의 기록 유형:<결정|액션|투두> 내용:<text> [담당자] [기한]`
  - `/회의 항목 동작:<조회|취소> [아이디:<id>]`
  - 요약 액션: `요약 재생성`, `결정 추가`, `액션 추가`, `할 일 추가`, `과제로 전환`, `원문 보기`
  - (호환) `/결정`, `/액션`, `/투두`, `/회의 항목조회`, `/회의 항목취소`
- `AssignmentSlashCommandHandler`
  - `/과제 등록` 기본: V2 모달(제목/마감일/링크 3필드) → ephemeral 2단계(채널/역할/멘션/알림프리셋) → 미리보기(`[등록 확정] [고급옵션] [취소]`)
  - `/과제 등록 제목/알림/마감 ...` 입력 시 레거시 즉시 등록(호환)
  - `/과제 목록`, `/과제 상세 아이디:<id>`, `/과제 완료 아이디:<id>`
  - 채널/역할/멘션은 길드 기본값(`default_task_channel_id`, `default_notify_role_id`)과 사용자 최근값으로 자동 채움
- `MogakcoSlashCommandHandler`
  - `/모각코 랭킹 기간:<day|week|month> [인원]`
  - `/모각코 내정보 기간:<day|week|month>`
  - `/모각코 오늘` (오늘 누적/출석체크 남은시간/1시간 목표 진행률)
- `AdminSettingSlashCommandHandler`
  - `/설정 마법사 [운영진역할] [회의열기역할] [홈채널] [회의채널] [모각코채널] [모각코음성추가] [모각코음성해제] [과제공지채널] [과제알림역할]`
  - `/설정 상태`
  - (호환/숨김) `/설정 운영진역할|회의채널|모각코채널|과제공지채널|과제알림역할 ...`
...
`/안건 설정`은 운영진 role 권한이 필요합니다.  
`guild_config`에 대상 길드의 `admin_role_id`가 있어야 합니다.

## 회의 시작 권한 준비

- 운영진 외 역할에도 회의 시작 권한이 필요하면 `/설정 마법사 회의열기역할:@1팀`으로 지정합니다.
- 확인: `/설정 상태`

## 기능별 공지 채널 분리

- 권장: `/설정 마법사`로 회의/모각코/과제 채널을 한 번에 설정
- 예: `/설정 마법사 운영진역할:@운영진 홈채널:#홈 회의채널:#회의 모각코채널:#모각코 모각코음성추가:#모각코-1 과제공지채널:#과제 과제알림역할:@알림역할`
- 조회: `/설정 상태`
- 홈 채널 가드:
  - 홈 채널에서 `/안건`, `/회의 시작`, `/회의 기록`/`/회의 항목`(호환 `/결정`/`/액션`/`/투두` 포함), `/모각코(랭킹/내정보/오늘)`, `/과제`를 실행하면 전용 채널로 안내
  - 전용 채널 미설정 시 `/설정 마법사`로 먼저 설정하도록 안내

## Mogakco 집계 방식

- 모각코로 등록된 음성채널만 집계합니다.
- 입장 시 `voice_sessions.joined_at` 생성, 퇴장/이동 시 `left_at`, `duration_sec`를 저장합니다.
- 세션 종료 시 `voice_session_daily_rollups`에 일자별 누적 시간을 upsert합니다.
- 통계 조회는 과거 일자는 롤업, 당일/열린 세션은 `voice_sessions`를 함께 사용해 합산합니다.
...
- [ ] `/핑` 동작 확인
- [ ] `/안건 설정` -> `/안건 오늘` 동작 확인
- [ ] `/설정 마법사 모각코채널:#모각코 모각코음성추가:#모각코-1` 설정 확인
