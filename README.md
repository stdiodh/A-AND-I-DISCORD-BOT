...
- 운영진 외 역할에도 회의 시작 권한이 필요하면 `/설정 회의열기역할 역할:@1팀`으로 지정합니다.
- 해제: `/설정 회의열기해제`
- 조회: `/설정 회의열기조회`

## 기능별 공지 채널 분리

- 회의 공지 채널: `/설정 회의채널 채널:#회의`
- 모각코 공지 채널: `/설정 모각코채널 채널:#모각코`
- 과제 공지 채널: `/설정 과제공지채널 채널:#과제`
- 조회: `/설정 채널조회`
- 홈 채널 가드:
  - 홈 채널에서 `/안건`, `/회의 시작`, `/결정`/`/액션`/`/투두`, `/모각코(랭킹/내정보/오늘)`, `/과제`를 실행하면 전용 채널로 안내
  - 전용 채널 미설정 시 해당 `/설정 ...채널` 명령으로 먼저 설정하도록 안내
