...
- `/회의 안건등록 링크:<http/https> 제목:<선택>`
  - 설명: 오늘 회의 안건 링크를 등록/수정
  - 권한: 운영진
...
- `/과제 상세 과제아이디:42`
  - 설명: 과제 상세 확인(알림역할/마감시각/임박알림/종료메시지 포함)
  - 권한: 전체
...
- 대시보드 빠른 등록(버튼)
  - 입력: 제목/링크/알림시각/마감시각/채널
  - 채널 입력 예시: `#과제공지` 또는 `123456789012345678`
  - 참고: 빠른 등록은 알림역할/종료메시지 기본값(없음)으로 저장
...
- `/설정 운영진조회`
  - 설명: 현재 운영진 역할 조회
  - 응답: 미설정 상태면 설정 방법(`/설정 운영진역할 대상역할:@운영진`) 안내
  - 권한: 전체 (권장: ephemeral)
