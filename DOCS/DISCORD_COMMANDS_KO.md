...
- 홈 구성 (HOME_V2 활성 기준)
  - 1행: `[회의 이동] [과제 이동] [모각코 이동]` (고정)
  - 2행: `더보기` 셀렉트
    - 안건 설정
    - 내 기록(개인)
    - 설정/도움말
  - 3행(설정 시): `오늘 안건 링크`
  - 채널 미설정 시 1행 버튼은 비활성 상태로 유지되어 레이아웃이 흔들리지 않음
  - `내 기록(개인)` 응답은 ephemeral로 표시되어 채널 스팸을 줄임
  - 홈에서는 채널 이동만 안내하고, 실제 공개 실행은 기능별 채널에서 진행
  - 홈 채널 가드:
    - 홈 채널에서 `/안건`, `/회의 시작`, `/결정`/`/액션`/`/투두`, `/모각코(랭킹/내정보/오늘)`, `/과제` 실행 시 전용 채널로 이동 안내
    - 전용 채널 미설정 시 `/설정 회의채널|모각코채널|과제공지채널` 설정 안내
...
- `/회의 시작 채널:#회의채널`
  ...
  - 추가 동작:
    ...
    - 홈 버튼으로 시작 시 `/설정 회의채널` 설정값이 있으면 해당 채널로 생성
  - 권한: 운영진 또는 회의열기역할(미설정 초기에는 Manage Server/Administrator)
...
## 모각코 통계
- `/모각코 랭킹 기간:<day|week|month> 인원:<선택, 기본 10>`
  - 설명: 기간별 누적시간 랭킹
  - `/설정 모각코채널` 설정 시 해당 채널에 게시되고 실행 채널에는 안내만 표시
  - 권한: 전체

- `/모각코 내정보 기간:<day|week|month>`
  - 설명: 내 누적시간/참여일/참여율 조회
  - `기간:day`일 때는 오늘 누적/출석체크 남은시간/1시간 목표 진행률을 함께 제공
  - 권한: 전체

- `/모각코 오늘`
  - 설명: 오늘 모각코 출석/1시간 목표 진행률을 바로 조회
  - 권한: 전체
...
- `/설정 회의열기역할 역할:@1팀`
  - 설명: 회의 시작 권한 역할을 `guild_config.meeting_opener_role_id`로 설정
  - 권한: 운영진 또는 Manage Server/Administrator

- `/설정 회의열기조회`
  - 설명: 현재 회의 시작 권한 역할 조회
  - 응답: 미설정 상태면 설정 방법(`/설정 회의열기역할 역할:@1팀`) 안내
  - 권한: 전체 (권장: ephemeral)

- `/설정 회의열기해제`
  - 설명: 회의 시작 권한 역할 설정을 해제(`meeting_opener_role_id = null`)
  - 권한: 운영진 또는 Manage Server/Administrator

- `/설정 회의채널 채널:#회의`
  - 설명: 회의 시작/요약 공지 채널 설정 (`meeting_board_channel_id`)
  - 권한: 운영진 또는 Manage Server/Administrator
...
- `/설정 모각코채널 채널:#모각코`
  - 설명: 모각코 랭킹 공지 채널 설정 (`mogakco_board_channel_id`)
  - 권한: 운영진 또는 Manage Server/Administrator
...
- `/설정 과제공지채널 채널:#과제`
  - 설명: 과제 공지 채널 설정 (`default_task_channel_id` 재사용)
  - 권한: 운영진 또는 Manage Server/Administrator
